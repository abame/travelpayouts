...
class HotelService extends AbstractService implements ServiceInterface, HotelServiceInterface
{
    ...
    public function getCostOfLiving(
        ...
        $dataService = $this->getDataService()->setClient(new Client($this->getClient()->getToken()));
    ...
    public function getHotelCollectionsTypes(int $id): array
    {
        ...
        $response = $this->getClient()->execute($url, ['id' => $id]);
    ...
    public function getRoomTypes(string $language = 'en'): array
    {
        ...
        $response = $this->getClient()->execute($url, ['language' => $language]);
        if (!is_array($response)) {
            throw new Exception('Response is not valid');
        }
        return $response;
    ...
    private function createHotelLocation(array $item): HotelLocation
    {
        return (new HotelLocation())
            ...
            ->setHotelCount((int)$item['hotelsCount'])
